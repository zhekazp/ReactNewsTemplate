...
const LogIn: React.FC = () => {
  ...
  const [recoveryModal, setRecovery] = useState(false);
  const [loading, setLoading] = useState(false);
  const [messageModal, setMassageMod] = useState<{
    title: string;
    content: string;
  }>({
    title: "Beachten Sie",
    content: "Geben Sie Ihre E-Mail-Adresse und Ihr neues Passwort ein.",
  });
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setMassageMod({
            title: "Antwort",
            content: "Anweisungen wurden an Ihre E-Mail gesendet.",
          });
          setRecovery(true);
        } else {
          setLoading(false);
          setMessage("Falscher Login data");
        }
      } catch (error) {
        setLoading(false);
        console.error(error);
        ...
        <div className="form-panel one">
          ...
          <div className="form-content">
            <div className="row g-3 needs-validation">
              ...
              {!(messageModal.title === "Antwort") && (
                <>
                  {loading ? <Spinner color={"white"} show={loading} /> : 
